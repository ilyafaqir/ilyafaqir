<!-- Header animé (effet "typing") -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=28&duration=3000&pause=1200&color=2b2b2b&center=true&vCenter=true&width=900&lines=Salut+%F0%9F%91%8B+Je+suis+Ilyas+Faqir;Data+Scientist+%7C+Data+Analyst;Machine+Learning+%7C+Deep+Learning;LLM+%7C+NLP+%7C+RAG" alt="Typing SVG" />
</p>

---

### 👋 À propos de moi

- 🎓 Passionné par la **Data Science**, le **Machine Learning** et les **LLMs**  
- 💡 J’aime créer des projets autour du **NLP**, du **RAG** et de l’**IA appliquée**  
- ⚙️ Expérience avec : `Python`, `TensorFlow`, `PyTorch`, `LangChain`, `Ollama`, `MongoDB`, `Docker`  
- 🚀 Toujours curieux d’explorer de nouvelles technologies  

---

### 📫 Me contacter

- 💼 [LinkedIn](https://www.linkedin.com/in/ilyas-faqir)  
- 📧  ilyas.faqir@edu.sorbonne-paris-nord.fr

---

⭐️ *N’hésite pas à visiter mes dépôts pour découvrir mes projets IA et Data Science !*
